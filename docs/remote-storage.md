...
| `keep_last` | `int` | `false` | `0` | Retention pruning of this target's uploaded objects. `0` = keep all (no pruning). `1+` = retain that many most-recently-modified archives, deleting older ones. A negative value is a no-op — logged as a warning, nothing is deleted. |
