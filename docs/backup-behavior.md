...
- With `keep_last: -1`, a partial run deletes local copies for that level, the same as a clean run, but only after at least one remote upload for the run succeeds (if every upload fails, the run fails before local cleanup, so the local copies are kept).
- In-progress (mid-write) archives are written as `*.tgz.incomplete` and are swept automatically at the start of the next run if orphaned.
